...
|IF
|Instruction Fetcher
|Module that contains actual and spculative PC and fetches instructions

|RAU
|Re-Allign Unit
|Module that realligns instructions and sent early branch prediction request

|DEC
|Decoder
|Module that decodes machine code instructions into internal control signals
...
| TRMC1: Reallign
| Instructions selected from IQ are realligned, branch prediction process is started if any instruction is branch

| TRMC2: Decode
| Instructions from RAU are decoded, target PC is calculated, branch prediction process is finished
